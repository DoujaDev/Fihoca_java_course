...
public class ProductoServicesImpl implements ProductoServices {
	...
	public List<Producto> getByFamilia(Familia familia) {
		...
		// Implementación "clásica" de un interface funcional
		
		Predicate<Producto> predicate = new Predicate<Producto>() {
			@Override
			public boolean test(Producto p) {
				return p.getFamilia() == familia;
			}	
		};
		
		
		// Implementación de un interface funcional con una lambda
		
		Predicate<Producto> predicate2 = x -> x.getFamilia() == familia;
		
		List<Producto> productos = PRODUCTOS.values()
				.stream()
			//	.filter(predicate)
